...
def _draw_rectangle(layer, position, color, name, shadow=1):
    ...
    rectangle.setAttribute("fill-opacity", str(shadow))
    rectangle.setAttribute("x", str(position[0]))
    rectangle.setAttribute("y", str(position[1]))
    layer.appendChild(rectangle)

def _draw_triangle(layer, position, color, name, corner = 'rb', shadow=1):
    ...
    triangle.setAttribute("fill-opacity", str(shadow))
...
def _draw_small_triangle(layer, position, color, name, corner = 'rb', shadow=1):
    ...
    small_triangle.setAttribute("fill-opacity", str(shadow))
...
def draw_nuclide(nuclide, layers, position, args):
    ...
    if args.ProdList is not None:
        ...
        if str(nuclide) not in args.ProdList:
            _draw_rectangle(layers[0], position,
                            primary_color, '{}0'.format(nuclide), 0.15)

        elif str(nuclide) in args.ProdList:
            _draw_rectangle(layers[0], position,
    ...
    if tertiary_size == 'small':
        if tertiary_color == COLORS['a']:
            ...
            corner = 'rb'
        if args.ProdList is not None:
            if nuclide not in args.ProdList:
                _draw_small_triangle(layers[1], position, tertiary_color,
                       '{}1'.format(nuclide), corner, 0.25)

            elif nuclide in args.ProdList:
                _draw_small_triangle(layers[1], position, tertiary_color,
                       '{}1'.format(nuclide), corner)
        else :
            _draw_small_triangle(layers[1], position, tertiary_color,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--Produced', action='append', 
                        dest='ProdList', type=str, help='Produced element list')
